...
即：在量化 clip 范围内，梯度直通（等于 1）；范围外为 0。这样可以用正常反向传播训练 scale 和 zero-point，以及对量化友好的权重分布。 这意味着梯度可以完美穿透 round，传递给原始权重 $x$。此时，优化器（SGD/Adam）会根据下游任务的 Loss，微调权重的数值。$\text{clamp}$ 函数的数学性质决定了，一旦越界被削平，无论 $x$ 怎么微小变动，输出都是固定的边界值（比如 127）。因此这里的梯度是绝对的 0。 这不仅符合数学逻辑，在工程上更是一种极佳的保护机制：那些极端离群值不会产生无穷大的梯度去“炸毁”整个网络的训练，它们被静默处理了
