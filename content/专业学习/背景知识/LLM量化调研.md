...
$z$ (零点偏移 Zero-point)：一个整数。它的核心作用是保证浮点数中的 $0.0$ 经过映射后，能够精确地落在一个整数上（即 $z$）。如果浮点分布不对称（例如 ReLU 的输出全是正数），加入 $z$ 可以进行非对称量化（Asymmetric Quantization），从而充分利用目标整数的比特位。

#### 为什么必须精确映射 0？

在 LLM 推理中，输入向量 $X$ 中出现纯 0.0 是极其高频的。假设算出来 $z = 12.3$，为了存进 INT8，我们把它四舍五入成了 $q_{zero} = 12$。那么当真实输入 $x_k = 0.0$ 时：硬件中执行的等效去量化操作就变成了： $(12 - 12.3) = -0.3$。这意味着，原本一个应该静默的 Padding 位或者被 Mask 掉的无关词，在每次矩阵相乘时，都会产生一个等效于真实世界 $-0.3 / s_x$ 的“幽灵数值”。如果恰好是精确整数，在GEMM过程中就能精确抵消掉。
...
#### 数据流细节
**W8A8 模式** 推理中最核心的 **重定标（Rescaling）与算子融合** 逻辑：

量化是为了加速密集的矩阵乘法，W8A8输出结果通常累加到 INT32，再转回 FP16 输出给下一层。这里需要“反量化”，以便进行加法（残差）和复杂的非线性运算。比如两层之间通常隔着 LayerNorm/RMSNorm 和 残差连接，这些算子在 INT8 下会造成巨大的精度损失。为了进入下一个 INT8 矩阵乘法单元，FP16 必须重新量化为 INT8。


为什么要重新算一遍：因为激活值（Activation）是动态的。第一层输出的范围可能是 [-2.5, 4.0]，而经过某些运算后，第二层的输入范围可能变成了 [-10.0, 12.0]。如果你沿用旧的 Scale，会导致严重的截断或精度丢失。因此，推理引擎通常会在每一层矩阵乘法之前，插入一个专门的 Quantize 算子，实时捕获当前 FP16 Tensor 的分布，将其重新映射到 INT8。

W8A8 的本质是**计算在 INT8，总线在 FP16**。它利用了整数计算的高吞吐量，同时利用浮点数作为“高精度中转站”，解决了神经网络中复杂算子无法被简单量化的难题。虽然多出了反量化和再量化的步骤，但由于这些算子通常是 Element-wise（逐元素） 的，计算量远小于矩阵乘法，所以整体上依然能获得巨大的速度收益。
...
在神经网络的训练中，模型能够“学习”的唯一途径是反向传播（Backpropagation）。反向传播极度依赖微积分中的链式法则（Chain Rule），也就是计算每一层对误差的梯度（导数）.因为 round 函数的几何形状像阶梯，导致梯度传到这里就断了，底层的权重收不到更新信号，整个网络直接“脑死亡”

STE也即在量化 clip 范围内，梯度直通（等于 1）；范围外为 0。这样可以用正常反向传播训练 scale 和 zero-point，以及对量化友好的权重分布。 这意味着梯度可以完美穿透 round，传递给原始权重 $x$。此时，优化器（SGD/Adam）会根据下游任务的 Loss，微调权重的数值。$\text{clamp}$ 函数的数学性质决定了，一旦越界被削平，无论 $x$ 怎么微小变动，输出都是固定的边界值（比如 127）。因此这里的梯度是绝对的 0。 这不仅符合数学逻辑，在工程上更是一种极佳的保护机制：那些极端离群值不会产生无穷大的梯度去“炸毁”整个网络的训练，它们被静默处理了
...
这个公式的内核，其实就是大模型在训练时计算的交叉熵损失（Cross-Entropy Loss）。 核心算子：$P(x_i | x_{<i})$ 代表模型的“自信程度”。给定前面所有的上文 $x_{<i}$，模型计算出下一个真实词 $x_i$ 的概率。如果模型非常聪明，它预测真实词的概率是 $0.99$，如果模型在胡说八道，它给真实词分配的概率可能只有 $0.01$。

我们希望整句话里，每一个真实词的概率连乘起来 $P(x_1) \times P(x_2|x_1) \times P(x_3|x_{<3}) \dots$ 越大越好。但直接连乘会带来两个致命的工程问题：数值下溢（Underflow）：几千个小于 $1$ 的浮点数相乘，结果会迅速逼近 $0$，最终超出 FP16 或 BF16 的表示下限。长度惩罚：句子越长，连乘结果必然越小，导致无法公平比较不同长度的句子。

为了解决浮点数下溢，我们在数学上套一个自然对数（$\ln$，在机器学习公式中通常简写为 $\log$）。根据对数的性质 $\log(A \times B) = \log(A) + \log(B)$，我们将概率的连乘变成了对数的连加。因为概率 $P$ 永远在 $[0, 1]$ 之间，所以算出来的 $\log P$ 会是一个负数。概率越接近 $0$，这个负数越小（比如 $\ln(0.01) \approx -4.6$）；概率越接近 $1$，这个负数越接近 $0$（比如 $\ln(0.99) \approx -0.01$）。

我们把句子里 $N$ 个词的对数概率加起来，除以 $N$ 算出一个平均值，这样就消除了句子长度带来的影响。最后，在前面加上一个负号，把负数变成正数。它在数学上完全等价于自回归语言模型的交叉熵损失。也就是说，模型在前向传播中算出来的 Loss 值，就是这部分。Loss 越小，说明模型整体给正确答案分配的概率越高。

在外面套一个指数函数 $\exp(x)$是为了让人类直观感受，执行 $\log$ 的逆运算，把数值强行拉回一个具有物理意义的维度——等效词表大小。当算出 $PPL = 10$ 时，它的物理意义是：模型在每一步预测下一个词时，就好像在从 $10$ 个等概率的候选词里盲猜一样。PPL越小，对应的困惑度就越低。

在计算量化模型的 PPL 时，“正确值”（Ground Truth）不是没量化前（FP16/FP32）模型的输出概率，而是评测数据集（如 WikiText-2、C4）中实实在在的“下一个词”。把量化模型的输出跟没量化的输出作对比叫 KL散度（KL Divergence） 或 Logits MSE。 FP16 老师也不是绝对的真理，FP16 模型在海量数据上训练到最后，不可避免地会记住一些特定的噪声，产生一定程度的过拟合。量化是有可能出现量化后PPL改善，但是与原量化之前模型存在差异的。
...
精度回退（Precision Fallback）是指当某层或某 batch 的量化误差超过阈值时，自动切换到更高精度的计算。在真实的底层工程实现中，我们绝对不会在推理时去动态计算“量化误差（如 MSE）”。 因为如果想在推理时动态知道当前 batch 的量化误差 $\epsilon = |Y_{FP16} - Y_{INT8}|$，就必须在底层既跑一遍 FP16 的矩阵乘法，又跑一遍 INT8 的矩阵乘法，然后再做差。这样就没必要量化了。

#### 动态回退（运行时）

在 vLLM 的 FP8 实现中，部分场景会根据 GPU 能力动态回退。 比如基于阈值的特征级回退，它的触发条件不是“误差”，而是“激活动态范围（Outlier Magnitude）”。评估确实是动态、逐 Batch、甚至逐 Token 进行的，但这极其考验底层 Kernel 的设计与硬件软硬件协同调优能力。判断逻辑（找离群值）：在前向传播时，当激活张量 $X$ 准备进入量化算子前，CUDA Kernel 会实时扫一遍 $X$ 中的元素。如果发现某个特征维度（Channel）的绝对值超过了一个预设的阈值（比如 $6.0$），就判定它为异常离群值。

动态路由与重组：
 - 剥离：将包含离群值的少数几个 Channel 及其对应的权重单独剥离出来，保持 FP16 格式。
 - 常规计算：剩下 99% 的正常数值被量化为 INT8。
 - 双流计算：在 GPU 上，FP16 的部分交给标准的 FP16 Tensor Core 算（极其稀疏）；INT8 的部分交给 INT8 Tensor Core 算（极其密集）
 - 结果融合：将两边的输出结果累加拼接
 
 动态回退在系统底层面临的问题是：
 - Kernel Launch 开销与同步墙：动态切分数据意味着需要额外的 Reduce/Scan 操作来找最大值，这会打断原本流水线般顺畅的算子执行。如果切分后还要启动两个不同的 GEMM Kernel，Kernel Launch 的开销甚至可能吃掉量化带来的微小收益。
 - 显存碎片化与显存带宽竞争：原本整齐划一的连续内存读取，变成了需要 Gathering 和 Scattering 的非连续访存。这极易导致 L2 Cache 的高频 Miss 和内存总线带宽的严重浪费，使系统迅速陷入 Memory Bound 的瓶颈。
 - Warp Divergence 隐患：如果试图把这种动态判断融合（Fuse）进同一个 Custom Kernel 里，GPU 线程束（Warp）内部的分支发散处理会变得异常复杂，极易影响 SM（流多处理器）的满载率。
...
静态回退 (Static Fallback)，基于 Offline Profiling 的层级回退（这是目前在 TensorRT-LLM 或 vLLM 等主流推理框架中最成熟、应用最广的做法）：
- 执行时机：部署前的校准阶段（Calibration）。

- 判断逻辑：系统会用几百条真实数据跑一遍 FP16 模型，再跑一遍 INT8 模型，按层（Layer）记录量化误差（如 KL散度或 Cosine Similarity）。

- 回退策略：如果发现某些层（例如 Transformer 的第一层、最后一层 LM Head，或者某些特定的 Attention 投影层）的误差超过了设定的阈值，系统就会在编译计算图时，静态地将这些层“钉死”在 FP16 精度。

- 底层表现：推理引擎在执行时，跑到这一层就直接调 FP16 的 GEMM Kernel，跑到其他层调 INT8 的 Kernel。没有任何动态判断的开销。
...
3. 伪量化提供了一个可微的（通过 STE，Straight-Through Estimator，直通估计器）量化误差仿真
